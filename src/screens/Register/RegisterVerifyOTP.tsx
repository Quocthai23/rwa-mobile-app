...
function RegisterVerifyOTP({
  navigation,
  route,
}: RootScreenProps<Paths.RegisterVerifyOTP>) {
  ...
  const onSubmit = async () => {
    ...
    try {
      setIsLoading(true)
      setOtpError('')
      const response = await authApi.verifyOTP(email, otp, 'register')
      console.log('🚀 ~ onSubmit ~ response:', response)
      navigation.navigate(Paths.RegisterCreatePassword, {
        email: email,
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      const response = await authApi.sendOTP(email, 'register')
